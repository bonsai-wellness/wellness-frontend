...
describe("Espacio Form Filling", () => {
  it("Completes Form Fields", () => {
    ...
    cy.get("#espacio-form-deporte").click();
    ...
    cy.get("#espacio-form-codigo").click();
    cy.get('nz-select[nzPlaceHolder="Puntos importantes"]').click();
    cy.get('nz-option-item[ng-reflect-label="Llevar raqueta"]').click();
    cy.get("#espacio-form-codigo").click();
    cy.get('input[placeholder="Detalles"]').type("Cancha para jugar padel");
